...
// migrateYamlToYml migrates old .yaml files to .yml extension
func migrateYamlToYml(path string) {
	// Only proceed if path ends with .yml
	if !strings.HasSuffix(path, ".yml") {
		return
	}

	// Check for old .yaml version
...
func generateConfigYAML(cfg *Config) string {
	return fmt.Sprintf(`# Quotes API Server Configuration
# Documentation: https://quotes.apimgr.us/docs
